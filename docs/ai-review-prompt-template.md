...
- Selbstverrat-Check (cloze, alle Varianten): Keine Lücke darf ihre Antwort im sichtbaren Aufgabentext vorwegnehmen. Ein akzeptierter Antwort-String darf weder im prompt noch im sentence außerhalb der Lücke wörtlich vorkommen. Typischer Fehler: eine Klammer-Gloss direkt neben der Lücke ("React ist eine ___ (Bibliothek)" bei akzeptierter Antwort "Bibliothek"). Auch der hint darf umschreiben und übersetzen, aber keinen akzeptierten String wörtlich nennen. Ausnahme: Werte, die die Aufgabenstellung zwingend vorgeben muss, damit die Aufgabe lösbar ist (z. B. der konkrete Zielwert einer Syntax-Übung). Schweregrad: KRITISCH.
